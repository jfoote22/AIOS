...
export type ModelSlot = 'openai' | 'claude' | 'anthropic' | 'fable' | 'grok' | 'gemini' | 'hermes';
...
export const SLOT_LABELS: Record<ModelSlot, { provider: string; variant: string; emoji: string }> = {
  ...
  fable:     { provider: 'Anthropic', variant: 'Fable',   emoji: '🔮' },
  grok:      { provider: 'xAI',       variant: 'Grok',    emoji: '⚡' },
  gemini:    { provider: 'Google',    variant: 'Gemini',  emoji: '✨' },
  hermes:    { provider: 'Hermes',    variant: 'Gateway', emoji: 'H' },
};

let cache: Record<ModelSlot, string> = { openai: '', claude: '', anthropic: '', fable: '', grok: '', gemini: '', hermes: '' };
let defaults: Record<ModelSlot, string> = { openai: '', claude: '', anthropic: '', fable: '', grok: '', gemini: '', hermes: '' };
