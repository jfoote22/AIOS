...
import {
  Terminal as TerminalIcon, Plus, X, Bot, Sparkles, AlertCircle,
  Square, Columns2, Columns3, Rows2, Grid2x2,
} from 'lucide-react';
...
export default function TerminalTab() {
  ...
  const spawnInSlot = useCallback(async (slotIndex: number, initialCommand?: string, labelHint?: string) => {
    ...
    try {
      ...
      const { id, shell, cwd } = await window.aios.term.spawn({ cols: 80, rows: 24 });
...
function NewButton({ icon, label, onClick, disabled }: { icon: React.ReactNode; label: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      title={disabled ? 'All visible slots are filled' : `New ${label} session`}
      className="flex items-center gap-1 px-2 py-1 rounded text-[10px] text-zinc-300 hover:text-white hover:bg-zinc-800 uppercase tracking-wider disabled:opacity-40 disabled:cursor-not-allowed"
    >
      {icon}
      {label}
    </button>
...
function Slot({
  slotIndex, session, containerRef, onSpawn, onClose,
}: {
  slotIndex: number;
  ...
  if (!session) {
    return (
      <div className="rounded-md border border-dashed border-zinc-800 bg-zinc-900/20 flex items-center justify-center">
...
function SmallButton({ icon, onClick, children }: { icon: React.ReactNode; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1 px-1.5 py-1 rounded text-[10px] text-zinc-400 hover:text-white hover:bg-zinc-800 uppercase tracking-wider border border-zinc-800"
    >
      {icon}
      {children}
    </button>
