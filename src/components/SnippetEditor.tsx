...
import { X, Copy, LayoutGrid, GripVertical } from 'lucide-react';
import {
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className={`mt-4 grid gap-3 ${compact ? 'grid-cols-3' : 'grid-cols-6'}`}>
          {item.subImages.slice(1).map((img, i) => {
            ...
            return (
              <div key={realIndex} className="aspect-square bg-zinc-800 rounded-xl border border-zinc-700 overflow-hidden group relative">
                ...
                <button onClick={() => removeSubImage(realIndex)} className="absolute top-1 right-1 p-1 bg-black/60 hover:bg-red-500/80 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity" title="Remove this capture"><X className="w-3 h-3" /></button>
              </div>
            );
          })}
          <button
